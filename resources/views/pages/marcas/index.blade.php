            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6">
                @foreach($brands as $brand)
                    <a href="{{ route('marcas.show', $brand->slug) }}"
                       class="group bg-white rounded-xl p-6 text-center shadow-sm hover:shadow-md transition border border-gray-100 hover:border-teal-200">
                        @if($brand->logo)
                        ...
                        @else
                            <div class="h-16 flex items-center justify-center mb-3">
                                <span class="text-2xl font-bold text-gray-300 group-hover:text-teal-500 transition">{{ strtoupper(substr($brand->name, 0, 2)) }}</span>
                            </div>
                        @endif
                        <p class="font-semibold text-gray-700 group-hover:text-teal-600 transition text-sm">{{ $brand->name }}</p>
                        @if(isset($brand->products_count) && $brand->products_count > 0)
                            <p class="text-xs text-gray-400 mt-1">{{ $brand->products_count }} productos</p>
                        @endif
                    </a>
                @endforeach
            </div>
