...
<nav class="bg-gray-50 border-b" aria-label="Breadcrumb">
    <div class="container mx-auto px-4 py-3">
        <ol class="flex items-center gap-2 text-sm text-text-muted">
            ...
            <li><span>/</span></li>
            <li><a href="{{ route('blog') }}" class="hover:text-brand-600">Blog</a></li>
            <li><span>/</span></li>
            <li class="text-gray-800 font-medium truncate">{{ $post->title }}</li>
        </ol>
    </div>
</nav>
    ...
    <div class="container mx-auto px-4">
        <div class="max-w-3xl mx-auto">
            ...
            <header class="mb-8">
                ...
                @if($post->excerpt)
                    <p class="text-xl text-text-muted leading-relaxed">{{ $post->excerpt }}</p>
                @endif

                @if($post->tags && count($post->tags) > 0)
                    <div class="flex flex-wrap gap-2 mt-4">
                        @foreach($post->tags as $tag)
                            <span class="text-sm bg-brand-50 text-brand-700 border border-brand-100 px-3 py-1 rounded-full">{{ $tag }}</span>
                        @endforeach
                    </div>
                @endif
            </header>
            ...
            <div class="mt-10 pt-8 border-t border-gray-100">
                <p class="text-text-muted text-sm mb-3">Compartir artículo:</p>
                <div class="flex gap-3">
                    <a href="https://wa.me/?text={{ urlencode($post->title . ' - ' . url()->current()) }}"
                       target="_blank" rel="noopener"
                       class="inline-flex items-center gap-2 text-sm bg-accent-500 text-white px-4 py-2 rounded-lg hover:bg-accent-600 transition">
                ...
                <article class="bg-white rounded-xl overflow-hidden shadow-sm hover:shadow-md transition">
                    ...
                    <div class="p-5">
                        ...
                        <h3 class="font-semibold text-gray-800 line-clamp-2">
                            <a href="{{ route('blog.show', $rel->slug) }}" class="hover:text-brand-600 transition">{{ $rel->title }}</a>
                        </h3>
                        <a href="{{ route('blog.show', $rel->slug) }}" class="inline-block mt-3 text-brand-600 text-sm hover:underline">
                            Leer →
                        </a>
                    </div>
                </article>
